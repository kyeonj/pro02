	...
	<form name="frm1" id="frm1" action="../InsertCustomCtrl" method="post" onsubmit="return joinCheck(this)">
		<table class="table">
			<tbody>
				<tr>
					...
					<td>
						<div class="form-row">
							...
							<input type="button" class="btn btn-light"" value="아이디 중복 확인" onclick="idCheck()">
				...
				<tr>
					<th>주소</th>
					<td><button id="post_btn" onclick="findAddr()" class="btn btn-light"">우편번호 검색</button>
		...
		<div class="btn-group">
			<input type="submit" name="submit-btn" class="btn btn-light" value="회원가입">
			<input type="reset" name="reset-btn" class="btn btn-light" value="취소">
			<a href="<%=request.getContextPath() %>/custom/login.jsp" class="btn btn-light">로그인</a>
		</div>
	</form>	
	<script>
	function idCheck(){
