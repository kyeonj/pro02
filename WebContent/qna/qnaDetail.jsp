			...
			<div class="btn-group">
				...
				<%
					if(sid.equals("admin")) {
						if(vo.getLev()==0){
				%>
					<a href='QnaReplyWriteCtrl.do?parno=<%=vo.getNo() %>' class="btn btn-outline-info">답변 하기</a>&nbsp;&nbsp;
					<a href='UpdateQnaCtrl.do?no=<%=vo.getNo() %>' class="btn btn-outline-info">글 수정</a>&nbsp;&nbsp;
					<a href='DeleteQnaCtrl.do?parno=<%=vo.getNo() %>' class="btn btn-outline-info">글 삭제</a>&nbsp;&nbsp;
				<%
						} else {
				%>
					<a href='UpdateQnaCtrl.do?no=<%=vo.getNo() %>' class="btn btn-outline-info">답글 수정</a>&nbsp;&nbsp;
					<a href='DeleteQnaCtrl.do?no=<%=vo.getNo() %>' class="btn btn-outline-info">답글 삭제</a>&nbsp;&nbsp;
				<%
						}
